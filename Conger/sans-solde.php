...
<body>
  ...
  <div class="content">
    ...
    if ($query->rowCount() > 0) {
      ...
      $prise = $resultat['Prise'] + htmlspecialchars($_POST['prise']);
      $reste = $resultat['Restant'] - htmlspecialchars($_POST['prise']);
      if ($reste >= 0) {
        $sql = "UPDATE conger SET Motif = ? , Prise = ? , Restant = ? WHERE Matricule = ? LIMIT 1";
        $req = $conn->prepare($sql);
        $update = $req->execute(array($motif, $prise, $reste, $matricule));
